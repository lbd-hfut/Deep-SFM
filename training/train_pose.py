#!/usr/bin/env python3
"""
Training script for DeepSFM pose subnet (PoseNet).
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train DeepSFM pose subnet")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/pose_default.yaml",
...
def create_model(config: Dict[str, Any], device: torch.device) -> nn.Module:
    """Create pose subnet model."""
    model_config = config["model"]
    
    model = PoseNet(
        translation_std=model_config["translation_std"],
        rotation_std=model_config["rotation_std"],
        n_pose_bins=model_config["n_pose_bins"],
